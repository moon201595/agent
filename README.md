...
현재 운영 환경은 Python 3.14와 저장소 루트의 `.venv`다.
...
일상 운영은 앞의 cron 경로다. 등록된 프로필을 수동으로 스캔하려면 같은 진입점을 쓴다. `--send`를 빼면 다이제스트를 생성·저장하지만 메일은 보내지 않는다. 검색·요약은 실행되므로 읽기 전용 명령은 아니다.
...
이 목록은 `docs/PROGRESS.md` §8 이 최신이다 — 이 파일은 갱신이 늦을 수 있으니 날짜 있는 최신 상태는 그쪽을 볼 것. 2026-09-09 기준 README 독자가 알아야 할 남은 제약은 다음과 같다.

- **검색 씨앗의 수율과 주제 다양성은 더 봐야 한다.** S2 후보가 채점에 쓰는 core 문자열과 잘 맞지 않는 문제가 있고, 새 씨앗의 적중률·고유 논문 수·소요 시간은 개정 후 운행으로 확인해야 한다. 위 하루 실측과 별개다(PROGRESS §8-73·74, §8-79 관련 기록).
- **S2 시간 예산은 엄격한 전체 실행 상한이 아니다.** `SEARCH_BUDGET_SECONDS=300`은 키워드·페이지 사이에서 경과 시간을 확인한다. 이미 진행 중인 요청의 HTTP 시간·페이싱·일반 오류 재시도를 같은 마감으로 강제 중단하지 않는다. 예산 초과의 실제 규모는 미실측이다(PROGRESS §8-75).
- **키워드별 자리 상한의 타당성은 미확정이다.** `KEYWORD_SLOT_SHARE=0.5`는 초기값이며, 다양성이 늘어나는지와 관련도 낮은 후보가 대신 올라오는지를 며칠 운행으로 봐야 한다(PROGRESS §8-62).
- **페이월 논문 전문은 무료 경로로 확보하지 못할 수 있다.** 본문·초록·서지정보의 수준을 구분하며, 자동 전문 수집 실패를 논문이 없다는 뜻으로 읽지 않는다. 합법적으로 확보한 PDF는 수동 업로드한다.
- **결합 가점이 두 적중에서 포화한다.** 서로 다른 core 를 세 개 맞혀도 두 개 맞힌 것과 동점이다(`profile_scoring.py` 의 `min(len(core_hits) - 1, 1)`). 여러 연구축이 만나는 논문을 그만큼 더 위로 올리지 못한다. 상한을 올릴 근거는 아직 없다 — 지금 코드는 "연구축"이 아니라 **적중 문자열 수**를 세기 때문이다(PROGRESS §8-79 의 남은 한계).
- **cron 이 도는 날과 안 도는 날을 기계가 구분하지 못한다.** 절전·종료로 05:00 을 놓치면 그날 메일이 없는데, 그게 "새 논문이 없었다"인지 "안 깨어났다"인지 로그만으로 갈리지 않는다(PROGRESS §8-72). 당분간 이대로 두기로 했다.
- **요약 정확도는 수치 검증 통과율과 별개다.** 처리 시간은 이미 실측됐으므로 "측정 없음"이 아니다. 의미적 정확도·검색 회수율은 여전히 미실측이며, 발표나 문서에서 수치를 만들지 않는다.

⑦ 코드 재현, S2 키 등록, 39편 회귀 기준선, 문장 그라운딩·인용 그래프·하이브리드 검색·구조화 JSON 출력은 구현된 기능이다. 이를 옛 미완료 항목으로 남기지 않는다. 구현 이력과 실패 사례는 `docs/PROGRESS.md` §5·§8을 따른다.
