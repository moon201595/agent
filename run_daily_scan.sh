...
#
# 보고서 생성 실패가 스캔의 성패를 덮으면 안 되므로 `|| true` 는 유지한다 —
# 아래에서 스캔의 종료코드로 끝낸다.
.venv/bin/python scripts/morning_report.py > /dev/null 2>&1 || true

# **스캔의 종료코드로 끝낸다**(2026-09-07, ⑨). 그전에는 status 를 로그에만
# 찍고 마지막 줄이 `|| true` 라 **항상 0 으로 끝났다** — cron 은 실패한 날과
# 성공한 날을 구분할 방법이 없었고, 바깥에서 붙일 수 있는 감시(메일 알림,
# healthchecks.io 같은 무료 dead-man's switch)도 이 신호가 없으면 못 붙는다.
#
# 락 스킵 경로는 위에서 `exit 0` 으로 이미 끝난다 — 겹쳐서 건너뛴 것은
# 고장이 아니므로 성공으로 보고하는 게 맞다.
exit "${status:-1}"
