...
# ── S2 시드와 채점 가중치의 분리 (2026-09-09, §8-79)
#
...
def test_시드가_있으면_가중치를_올려도_질의어가_안_늘어난다():
    """이 테스트가 잡는 것: 명시적 시드 경로에 가중치 필터를 다시 연결하는 것.

    비시드 키워드를 전부 1.0 으로 올려도 질의 목록이 그대로여야 한다 —
    분리의 존재 이유가 바로 이것이다.
    """
    profile = {
    ...
    assert s2_delta.keywords_for_s2(profile) == ["world model"], (
        "가중치를 올렸더니 질의어가 늘었다 — 시드와 중요도가 다시 붙었다")


def test_시드가_비면_종전대로_가중치로_고른다():
    """이 테스트가 잡는 것: 하위 호환 경로 삭제.

    시드를 안 쓰는 구형 프로필은 예전과 똑같이 동작해야 한다. 시드가 비었다고
...
def test_시드에도_중복_제거_규칙이_그대로_걸린다():
    """이 테스트가 잡는 것: 시드 경로에서 S2_REDUNDANT_KEYWORDS 를 빠뜨리는 것.
    ...
    # 대신할 시드가 없으면 개념을 통째로 잃지 않게 그대로 둔다.
