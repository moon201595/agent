...
# 걸려 못 본 게 남으면 truncated 로 표시한다. **다만 상한(page_cap)은 partial 로
# 세지 않는다**(2026-09-08, §8-78 ②) — 그건 사고가 아니라 우리가 고른 범위이고,
# partial 로 남기면 창이 안 전진해 다음 날 같은 창을 다시 훑고 또 상한에 걸린다.
# 예산 소진·페이지 실패·offset 상한만 partial 이다. 상한에 걸린 사실은
# keywords_capped 와 query 의 capped×N 으로 남는다 — 기록에서 사라지지 않는다.
...
async def find_new_papers_since(
    ...
    #
    # **2026-09-08 정정 두 번.** (1) 원래 "done 은 래칫을 전진시키므로 그 창의
    # 나머지 논문은 다시 조회되지 않는다"고 적혀 있었는데, 그때는 S2 의 status 가
    # 커서 계산에 아예 안 쓰였다. (2) **그런데 그날 오후 §8-76 으로 쓰이게 됐다** —
    # 이제 이 status 는 창을 실제로 붙잡는다. 정정문이 다시 낡은 경우라 여기 남긴다.
    # 기록이 사실과 달라지면 나중에 그 기록으로 아무것도 설명할 수 없다.
    #
