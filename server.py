...
_config.apply_to_os()      # 2026-09-17: 파서는 config 하나

# ---------------------------------------------------------------- 상수/경로

BASE_DIR = Path(__file__).resolve().parent
# 경로는 storage.py 가 소유한다(2026-09-04) — 스키마와 파일 배치는 같은 관심사다.
DATA_DIR = storage.DATA_DIR
PDF_DIR = storage.PDF_DIR
TEXT_DIR = storage.TEXT_DIR
SUMMARY_DIR = storage.SUMMARY_DIR
IMAGE_DIR = storage.IMAGE_DIR
REPRO_DIR = storage.REPRO_DIR
...
# 테스트는 server.DB_PATH 를 monkeypatch 한다 — 옮기자마자 55개가 깨졌다.
# 함수면 호출 시점에 server.DB_PATH 를 읽으므로 기존 테스트가 그대로 돈다.
def _init_storage() -> None:
    storage.init_storage(DB_PATH)


def _db():
    return storage.db(DB_PATH)
...
# 외부 API 어댑터도 http_client 가 소유한다(2026-09-04) — MCP 와 무관한
