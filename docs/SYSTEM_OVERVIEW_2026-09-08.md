...
| ① S2가 못 본 긴 구간이 사라짐 | 실제 공통 창은 arXiv 이력만 사용; 5일 안전 창 밖의 S2 미조회 구간은 보호하지 못함 | 현재는 `min(arxiv since, s2 since)` | 과거 33회 재생: 실제 창 증가 2회, 최대 +8.2시간·평균 +4.3시간. **단 이 값은 S2 이력이 있는 경우만 재생한 것이다**(2026-09-08 저녁 외부 검토가 지적). 이력 없음 → 7일 규칙까지 넣으면 첫 S2 실행(09-03)은 +48시간이고, 창 길이 재생만으로 실제 페이지·호출 증가를 확정할 수도 없다 — 미실측이다. 수정 후 장기 API 비용도 미실측이다. **§8-76은 해결됐고 남은 세 가지는 §8-78 이다.** | PROGRESS:1607~1632, §8-76; `2b52da7` |
...
| ~~P1: 결과 절 파싱 경로 일치~~ **(2026-09-08 저녁 처리됨)** | 이 문서를 쓰며 발견한 것이다. `digest._split_sections` 만 확장돼 `summary_parser.parse_sections` 와 갈려 있었고, `trend_report.result_excerpts`·`get_summary_json` 이 후자를 써서 메일에 보이는 절이 동향 입력에서 빠졌다. | 두 파서를 같은 규칙으로 맞췄다. 절 깊이는 **문서에 나오는 가장 얕은 제목**으로 정하고(`###` 안의 `####` 가 부모를 비우던 것 포함), 불릿이 없으면 산문을 항목으로 쓴다. | **실측: 저장된 요약 131편 중 73편(56%)의 `결과` 절이 새로 살아났다.** 원인은 대부분 산문(126편이 `### 결과` 를 갖고도 불릿 0개였다). 커밋 `b97fc04`, PROGRESS §8-74 후속 |
