...
## 슬라이드 (`피드백_반영_보고_2026-09-20.pptx`, 14장, 16:9)
...
표지 → **I 이번 3일 요약(9/20 추가)** → II 피드백·반영 표(네이티브 표) → III 루프 → IV 가중치 → V 동향 서술 →
**VI 세 질문으로 갈랐다(9/20 추가)** → VII 주간 관리(월요일) → **VIII 월요일 메일 한 통(9/20 추가)** →
IX 재현 찾기 → X 재현 실행·정리 → XI 디스크 → XII 보안 → XIII 정리.

로마숫자는 `head()` 가 자동으로 매긴다 — 슬라이드를 중간에 끼워 넣을 때마다 손으로 다시 매기다 어긋났다(9/20).
시안·QA 는 Windows PowerPoint COM 으로 PNG 를 뽑아 눈으로 본다(WSL 에 LibreOffice 가 없다). 검증은
`pptx` 스킬의 `scripts/office/validate.py`(`defusedxml`·`lxml` 필요).
