...
결정적 도구 8종을 MCP 서버로 노출하고 에이전트 루프는 Claude Code 에 맡기는 구조가 동작한다. **MCP 연결까지 검증됐고, 논문 1편을 실제로 끝까지 태워 저장까지 확인했다(2026-07-30).** ⑥ 사람 판단, ⑦ 코드 재현은 아직 미착수다. **④ 는 `batch_summarize.py` 온디맨드 배치 스크립트로 구현 완료됐다(2026-07-31)** — 사람이 명령 한 번 실행하면 검색→선별→파싱→요약→검증→저장까지 채팅 승인 없이 돈다. 요약 엔진은 로컬 파인튜닝이 아니라 **무료 API(Gemini 우선, Groq 대체)** 로 방향이 바뀌었다 — §8-2 참고.
...
`batch_summarize.py` 는 `server.py` 의 MCP 도구 함수를 `import server` 로 그대로 불러와 직접 호출한다 — `@mcp.tool` 데코레이터가 있어도 일반 함수처럼 호출 가능함을 실측으로 확인했다. 로직 중복 없이 ①②③⑤와 저장 경로를 그대로 재사용한다.
...
`--keyword` 모드(검색→선별까지 포함한 전체 경로)는 로직 구현과 예외 처리(외부 API 429 시 죽지 않고 안내 메시지 출력)까지는 확인했으나, 이 세션에서 arXiv·Semantic Scholar 를 과다 호출해 실제 검색 결과로 끝까지 도는 라이브 테스트는 아직 못 했다 — 다음 세션에서 확인할 것.
...
2-1. **`--keyword` 모드 라이브 테스트** — 로직·예외처리는 확인됐으나 arXiv/S2 429 로 실제 검색→선별까지 도는 전 과정은 아직 못 봤다. 다음 세션에서 확인할 것 (외부 API 호출을 이 세션에서 과다하게 써서 일시적으로 막힌 것으로 추정, 재현성 문제 아님).
