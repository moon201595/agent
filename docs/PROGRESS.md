...
23. **⑦ 저장소 지목 오류가 두 종류 더 남아 있다** (2026-08-31, 개편 후 첫 운행 실측).
    19번(본문 URL 뒤에 문장이 붙는 것 + 도구 저장소 제외 목록)을 고친 뒤에도
    6편 중 3편이 엉뚱한 저장소를 잡았다. 원인이 19번과 **다르다**:
    - `label-studio`(2608.25176) — 제외 목록에 없는 **도구** 저장소. 논문이
      라벨링 도구로 언급한 것을 저자 저장소로 봤다. 19번에서 목록을 좁게
      유지한 대가다(넓히면 진짜 저자 저장소를 삼킨다고 판단했고 그 판단은
      지금도 유효하다) — 목록을 키우는 게 아니라 **판정 방식**이 문제다.
    - `orbitinghail/graft`(2608.27079) — **이름 충돌**. 논문 이름이 GRAFT 라
      `github_search` 가 동명의 무관한 저장소(스토리지 엔진)를 1등으로 줬다.
      본문에서 찾은 게 아니라 검색으로 찾은 것이라 19번 수정이 닿지 않는다.
    - `Riemann-1.0-Website`(2608.27033) — 저자 조직은 맞는데 **코드가 아니라
      웹사이트** 저장소다.
    공통점: 지금은 후보 하나를 뽑아 바로 클론한다. 필요한 건 "이게 이 논문의
    코드가 맞는가"를 클론 **전에** 확인하는 단계다. 후보군은 이미
    `find_repo_candidates` 가 `in_text`/`github_search` 로 나눠 돌려주고
    신뢰도까지 붙여 준다 — 그 정보를 쓰지 않고 있다.
    **후보 대응**: (a) `github_search` 결과는 저자명·논문 제목이 README 나
    저장소 설명에 나타날 때만 채택(이름 충돌 차단) (b) 저장소 이름이
    `-website`/`-page`/`.github.io` 로 끝나면 후순위 (c) `in_text` 후보를
    `github_search` 보다 항상 우선.
    **주의**: 이건 판정 로직이라 LLM 을 넣지 않는다(CLAUDE.md 7) — 전부
    문자열 대조로 가능하다. 아직 구현 안 했다.
