...
11. Groq 보충 프롬프트(`call_groq_addendum`)의 별점(★) 표기 흔들림 — 2026-08-06 실측 재검증 중 발견(§5 "서베이 템플릿·청킹 실측 재검증" 참고). 검증(숫자 대조) 자체엔 영향 없지만 `review_app.py`에서 읽을 때 `★등급`/`★중`/`★★★★` 같은 일관성 없는 표기가 섞여 나온다. 우선순위 낮음 — 표기 다듬기만 남은 사안.
